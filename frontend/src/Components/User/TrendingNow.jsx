...
const TrendingNow = memo(({ posts }) => {
  ...
  const slideRight = () => {
    if (currentIndex < posts.length - visibleCards) setCurrentIndex((prev) => prev + 1);
  };
      ...
      <div className="relative w-full max-w-7xl">
        ...
        <div className="overflow-hidden">
          <div
            ...
            {posts.map((blog, index) => (
              <div key={blog.id} className={`w-full md:w-1/${visibleCards} flex-shrink-0 px-6 lg:px-4`}>
                <div className="bg-white border rounded-xl shadow-md hover:shadow-lg overflow-hidden">
                  <Link to={`/${createSlug(blog?.categories[0]?.category_name)}/${createSlug(blog?.Custom_url)}`}>
                    <img
                      src={blog.featured_image ? `${import.meta.env.VITE_API_URL}/${blog.featured_image}` : "https://via.placeholder.com/300x200.png?text=No+Image"}
                      alt={blog.title}
                  ...
                  <div className="p-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-sm font-semibold text-gray-700">{blog.author_name}</p>
                      </div>
                      <time dateTime={blog.date} className="text-xs text-gray-400 font-semibold">
                        {new Date(blog.scheduleDate || blog.created_at).toLocaleDateString("en-US", {
                          year: "numeric",
                          month: "long",
                          day: "numeric",
                        })}
                    ...
                    <h2 className="text-lg font-bold my-2 line-clamp-2">{blog.title}</h2>
        ...
        <button
          ...
          className={`absolute right-[-20px] lg:right-[-50px] top-1/2 transform -translate-y-1/2 bg-[#FF822E] text-white p-3 rounded-xl shadow-md hover:scale-105 transition z-10 ${
            currentIndex >= posts.length - visibleCards && "opacity-50 cursor-not-allowed"
          }`}
