    ...
    const fetchData = async () => {
      try {
        ...
        const mainCatRes = await axios.get(
          `${BASE_URL}/categories?slug=risk-management` // Changed slug to "risk-management"
        );
        ...
        const grouped = await Promise.all(
          subCategories.map(async (subCat) => {
            ...
            return {
              category: {
                name: subCat.name,
                link: `/risk-management/${subCat.slug}`,
              },
              posts: postsRes.data,
            };
          })
    ...
    <>
      <div className="relative w-full h-[350px] lg:h-[400px] overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-center gap-3 py-5 px-[2%] lg:px-[10%] h-full text-white bg-black/40">
          <h1 className="lg:text-5xl text-xl font-semibold">Risk Management</h1>
          <p className="lg:text-base text-xs leading-relaxed">
            You do need to build wealth, but you also need to protect yourself
            if the worst happens. Our risk management at Trust Finance Why let
            your most precious of your assets be exposed. Look into insurance,
            special covers that will take care of your belongings, health and
            life. Maximize Your Golden Years With Wise Retirement Planning You
            should have the right plan to go boldly in the future.
          </p>
        </div>
      </div>
